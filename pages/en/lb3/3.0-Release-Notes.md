...
See the [3.0 Migration Guide](Migrating-to-3.0.html)
for instructions on upgrading existing LoopBack 2.x applications to 3.0.

## Changes to loopback-datasource-juggler

### loopback-datasource-juggler is now a regular dependency

LoopBack 2.x (ab)used peer dependencies to ensure there was only one instance
of `loopback-datasource-juggler` in the dependency tree, so that there would be only
one singleton instance of model registry. This was fragile and problematic in certain edge cases.

We have reworked loopback-datasource-juggler and connectors to not rely on
a single juggler instance anymore. As the last step, was changed from peerDependencies to dependencies.
...
The `DataSource.registerType()` method was removed as it was a deprecated wrapper
...
The `Dynamic` component was removed in favor of type converters and a
registry.
...
removed. Please use the `create` function instead (array inputs only worked to
...
hook, which is to allow hooks to modify the raw data provided by the datasource
...
For more information, see [how to migrate](Migrating-to-3.0.html#check-your-cors-configuration)

### Removed current context API and middleware
...
For more information, see [Using current context](Using-current-context.html).
...
and [Using current context](Using-current-context.html).

For more information, see [how to migrate](Migrating-to-3.0.html#the-current-context-feature-was-moved-to-loopback-context)
...
For more information, see [how to migrate](Migrating-to-3.0.html#new-error-handler-for-rest)
...
### Unused user properties removed
...
If your application is using these properties, you can define them in your
...
errors only. All Change methods now report errors to the caller via the callback.

To report change-tracking errors, use
`[PersistedModel.handleChangeError](http://apidocs.strongloop.com/loopback/#persistedmodel-handlechangeerror)`. You can customize this method to provide specific error handling for a given model.

For more information, see [loopback#2308](https://github.com/strongloop/loopback/pull/2308).

### Removed `loopback.autoAttach()` method

LoopBack 2.0, reworked the way LoopBack applications are
...
LoopBack 3.0 removes the `loopback.autoAttach` method and
`defaultForType` datasource option completely.
