...
First, you have to configure a data source using the [REST connector](/doc/{{page.lang}}/lb2/REST-connector.html).
In the [datasources.json](/doc/{{page.lang}}/lb2/datasources.json.html) file that configures the data source,
...
2.  Add the following to the `"files"` property in [`middleware.json`](/doc/{{page.lang}}/lb2/middleware.json.html):
...
* [Operation hooks](/doc/{{page.lang}}/lb2/Operation-hooks.html) that execute when the model performs CRUD (create, read, update, and delete) operations.
* [Remote hooks](/doc/{{page.lang}}/lb2/Remote-hooks.html)  that execute before or after a remote method is called.

### Does LoopBack support JavaScript configuration files (not just JSON)?

LoopBack does not support using JavaScript (instead of JSON) for configuration files such as [config.json](/doc/{{page.lang}}/lb2/config.json.html), 
[datasources.json](/doc/{{page.lang}}/lb2/datasources.json.html), or [model-config.json](/doc/{{page.lang}}/lb2/model-config.json.html), 
...
See [Managing users](/doc/{{page.lang}}/lb2/Managing-users.html) for more information and 
