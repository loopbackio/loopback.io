...
* iOS SDK (Objective C) for iPhone and iPad apps. See [iOS SDK](/doc/en/lb2/iOS-SDK.html) for more information.
* Android SDK (Java) for Android apps. See [Android SDK](/doc/en/lb2/Android-SDK.html) for more information.
* AngularJS (JavaScript) for HTML5 front-ends. See [AngularJS JavaScript SDK](/doc/en/lb2/AngularJS-JavaScript-SDK.html) for more information.
...
Additionally, there are [community connectors](/doc/en/lb2/Community-connectors.html) created by developers in the LoopBack open source community.
...
If you are trying [query filters](/doc/en/lb2/Querying-data.html) with curl, use the `-g` or `--globoff`
...
First, you have to configure a data source using the [REST connector](/doc/en/lb2/REST-connector.html).
In the [datasources.json](/doc/en/lb2/datasources.json.html) file that configures the data source,
...
1.  Configure a datasource to use the [email connector](/doc/en/lb2/Email-connector.html).
...
1.  Remove the contents of the default `"routes"` property in [`middleware.json`](/doc/en/lb2/middleware.json.html).
2.  Add the following to the `"files"` property in [`middleware.json`](/doc/en/lb2/middleware.json.html):
...
* [Operation hooks](/doc/en/lb2/Operation-hooks.html) that execute when the model performs CRUD (create, read, update, and delete) operations.
* [Remote hooks](/doc/en/lb2/Remote-hooks.html)  that execute before or after a remote method is called.
...
See [Managing users](/doc/en/lb2/Managing-users.html) for more information and 
...
you can [set a property in config.json](/doc/en/lb2/config.json.html#config.json-Top-levelproperties) to do so.