...
Replace the default `root.js` with [a new one](https://github.com/strongloop/loopback-example-relations/blob/master/server/boot/root.js)
which passes a given customer's id to template engine.
...
* belongs to
  * `Customer`
  * Property name for the relation: _Leave blank - defaults to `customer`_
  * Custom foreign key: _Leave blank_
...
* List a predefined scope named _youngFolks_
